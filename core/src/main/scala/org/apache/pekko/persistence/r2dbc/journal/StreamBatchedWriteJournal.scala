    ...
    val writeResult: Future[Instant] =
      JournalSerialization.serialize(atomicWrite, timestamp, serialization, persistenceExt) match {
        case Success(events) =>
          ...
          promise.future.map(_ => timestamp)(ExecutionContexts.parasitic)
        case Failure(exc) =>
          Future.failed(exc)
      }
    ...
    journalDao
      ...
      .transformWith {
        ...
        case Failure(exc) =>
          ...
          writeOrBisect(left).flatMap(_ => writeOrBisect(right))(ExecutionContexts.parasitic)
      }(ExecutionContexts.parasitic)
  }
